...
CREATE INDEX IF NOT EXISTS idx_provprod_proveedor ON proveedor_productos(proveedor_id);
CREATE INDEX IF NOT EXISTS idx_provprod_user      ON proveedor_productos(user_id);

ALTER TABLE proveedor_productos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "provprod_select" ON proveedor_productos FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "provprod_insert" ON proveedor_productos FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "provprod_update" ON proveedor_productos FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "provprod_delete" ON proveedor_productos FOR DELETE TO authenticated USING (auth.uid() = user_id);
